...
from dvae_model.prob_utils import normal_parse_params, rec_log_prob, standard_normal
...
class DvaeDynamics(object):
    ...
    def batch_vlb(self, s, ac, s_next):   
        ...
        prior_regularization = 0                                     # (None, None)
...
if __name__ == '__main__':
    ...
    print("\n------------\nInit dynamic.\n")
    dynamics = DvaeDynamics(auxiliary_task=feature_extractor, sample_seeds=10,
                            reward_type='elbo')
    ...
    vars_ = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="dvae")
    print(np.sum([np.prod(v.shape) for v in vars_]))

    print("Feature Extractor:")
    vars_ = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="inverse_dynamics")
    print(np.sum([np.prod(v.shape) for v in vars_]))
