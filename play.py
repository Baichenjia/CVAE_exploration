    ...
    def play(self, tf_sess, args_tmp, saver, model_path):
        ...
        for i in range(5):
            ...
            if max_reward < np.sum(rews):
                ...
                save_np_as_mp4(frames, "/Users/bai/Desktop/video/"+args_tmp['env']+'.mp4')
...
def add_environments_params(parser):
    ...
    # Mario: python play.py --env mario --env_kind mario
    # Multi Pong: nohup python run.py --env multi-pong --env_kind retro_multi > log_multi.txt 2>&1 &
    parser.add_argument('--env', help='environment ID', default="Robotank"+"NoFrameskip-v4", type=str)
    ...
    parser.add_argument('--stickyAtari', action='store_true', default=True)   # 是否增加环境模型的随机性
    parser.add_argument('--reward_type', type=str, default="elbo", choices=["kl", "elbo", "elbo_var", "pred_var", "var_mean"])
    parser.add_argument('--nepochs_dvae', type=int, default=3)
    parser.add_argument('--sample_seeds', type=int, default=10)
    parser.add_argument('--model_path', type=str, default="result/atari_sticky/VDM/Robotank_VDN_1/model_last.ckpt")
